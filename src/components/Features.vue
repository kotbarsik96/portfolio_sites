  ...
  <section id="features-block" class="page-block features-block">
    <div class="container">
      ...
      <div class="features-block__body">
        <div class="features-block__book-pages">Страницы {{ currentPages }} из {{ pagesTotal }}</div>
        <swiper
        :modules="modules"
        :effect="'flip'"
        class="features-book"
        ...
        >
            <swiper-slide class="features-book__slide" v-for="slide in twoPagesArray.video" :key="slide[0].id">
              <two-pages @createModalWindow="createModalWindow" :slide="slide" :presentsBy="'video'" :srcType="srcType"></two-pages>
            </swiper-slide>
            <swiper-slide class="features-book__slide" v-for="slide in twoPagesArray.img" :key="slide[0].id">
              <two-pages @createModalWindow="createModalWindow" :slide="slide" :presentsBy="'img'" :srcType="srcType"></two-pages>
            </swiper-slide>
        </swiper>
        <swiper
        :modules="modules"
        ...
        @slideChange="onSlideChange"
        @touchStart="onTouchStart">
            <swiper-slide class="features-book__slide" v-for="page in singlePageArray.video" :key="page.id">
              <single-page @createModalWindow="createModalWindow" :page="page" :presentsBy="'video'" :srcType="srcType"></single-page>
            </swiper-slide>
            <swiper-slide class="features-book__slide" v-for="page in singlePageArray.img" :key="page.id">
              <single-page @createModalWindow="createModalWindow" :page="page" :presentsBy="'img'" :srcType="srcType"></single-page>
            </swiper-slide>
        </swiper>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setCurrentPages(swiper){
      ...
      const pageElems = Array.from(activeSlide.querySelectorAll('[data-page-number]'));
      if(pageElems.length === 1) this.currentPages = pageElems[0].dataset.pageNumber.toString();
    ...
    setSrcType(){
      const onResize = () => {
        ...
        if(w < h && this.srcType !== 'src-vertical') this.srcType = 'src-vertical';
      };
    ...
    createModalWindow(ftr){
      console.log(ftr);
      const ftrTitle = `<span class="__selected">${capitalLetter(ftr.title)}</span>`;

      const bodyContent = ftr.examples.length < 2 ? createParagraph() : createList();
      ...
      function createList(){
        ...
        const links = [];
        ftr.examples.forEach(item => links.push(`<a href="${item.url}">${item.text}</a>`));
        const content = {
          bodyType: "text",
          title: wTitle,
          ol: links
